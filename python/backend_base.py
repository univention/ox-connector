...
def register_ox_integration_backend_class(backend, object_type, cls):  # type: (str, str, Type[OxObject]) -> None
	"""
	Backend class registration. Used by BackendMetaClass.

	:param backend: str - either 'CSV' or 'SOAP'
	:param object_type: str - one of 'Context', 'Group', 'Resource', 'User'
...
class OxObject(object):
	...
	def __init__(self, *args, **kwargs):  # type: (*str, **str) -> None
		self.logger = getLogger('ox.connector.object.{}'.format(self.__class__.__name__))
	...
	def from_ox(self, context_id, obj_id=None, name=None):  # type: (Optional[int], Optional[str]) -> OxObject
		"""
	...
	def list(self, context_id, pattern, *args, **kwargs):  # type: (*str, **str) -> [OxObject]
		"""
