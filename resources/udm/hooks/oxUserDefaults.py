...
class oxUserDefaults(simpleHook):
	...
	def ox_defaults(cls, lo, pos):
		if not cls._ox_defaults:
			...
			cls._ox_defaults = dict(isOxUser='Not')
			...
			ud.debug(ud.ADMIN, ud.ALL, "oxUserDefaults.ox_defaults() ox_defaults=%r" % cls._ox_defaults)
	...
	def hook_ldap_pre_modify(self, module):
		if module.has_property('oxDisplayName'):
			...
			for key, value in module.oxUserDefaults.iteritems():
