...
class oxAccess(simpleHook):
	...
	@staticmethod
	def log_info(msg):
		univention.debug.debug(univention.debug.ADMIN, univention.debug.INFO, 'admin.syntax.hook.oxAccess: %s' % msg)
	...
	def check_syntax(module):
		if module['username']:
			...
			unicode_username = username.decode("utf-8")
			p1 = re.compile('(?u)(^[a-zA-Z])[a-zA-Z0-9._-]*([a-zA-Z0-9]$)')
			p2 = re.compile('(?u)^\w([\w -.]*\w)?$')
	...
	def hook_ldap_pre_remove(self, module):
		...
		if resources:
			raise univention.admin.uexceptions.valueError(oxAccess._('The user %s cannot be removed because he is admin of the following resources: %s') % (module.oldinfo.get('username', '???'), ', '.join(resources)))
