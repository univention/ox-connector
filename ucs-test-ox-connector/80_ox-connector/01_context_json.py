...
def test_context_json_file():
    ...
    contexts_json_path = (
        ...
        '/secrets/contexts.json'
    )  # Path to the contexts.json file
    expected_keys = [
        "adminpass",
        "adminuser",
    ]  # Keys that should be present in each context entry
    ...
    try:
        ...
        with open(contexts_json_path, 'r') as file:
            try:
                ...
                assert isinstance(contexts_json, dict), (
                    "Invalid JSON format: contexts.json should "
                    "be a dictionary"
                )
            ...
            assert (
                'master' in contexts_json
            ), "Missing 'master' entry in contexts.json"

            # Get master password from /etc/ox-secrets/master.secret file
            master_secret_path = '/etc/ox-secrets/master.secret'
            assert os.path.exists(master_secret_path), (
                f"Error: Master secret file not f"
            ...
            if (
                'master' in contexts_json
                and 'adminpass' in contexts_json['master']
            ):
                ...
                for (
                    context_number,
                    context_credentials,
                ) in contexts_json.items():
                    if context_number != 'master':
                        if all(
                            key in context_credentials for key in expected_keys
                        ):
                            context_secret_path = (
                                f'/etc/ox-secrets/context'
                            ...
                            if os.path.exists(context_secret_path):
                                with open(
                                    context_secret_path,
                                    'r',
                                ) as context_secret_file:
                                    context_password = (
                                        context_secret_file.read().strip()
                                    )
                                json_adminpass = context_credentials[
                                    'adminpass'
                                ]
