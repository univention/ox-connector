...
def load_from_json_file(path):
    """
    Just a helper function to get JSON content from a file, if it exists
    """
    if not path.exists():
        return None
    ...
    def __repr__(self):
        if not self._old_loaded:
            return "Object({}, {}, {})".format(
                self.entry_uuid, self.object_type, self.dn
            )
        else:
            return "Object({}, {}, {}, {})".format(
                self.entry_uuid, self.object_type, self.dn, self.old_dn
            )
