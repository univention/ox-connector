...
def read_current_configuration():
    try:
        ...
        print("Current configuration not found. Using default mapping.")
        return DEFAULT_MAPPING


CURRENT_CONFIG = read_current_configuration()
...
def save():
    with open(
        ...
        json.dump(CURRENT_CONFIG, fd, indent=4)


def set_property(ox_property, ldap_attribute):
    CURRENT_CONFIG[ox_property]["ldap_attribute"] = ldap_attribute


def unset_property(ox_property):
    nillable = CURRENT_CONFIG[ox_property]["nillable"]
    if nillable:
        CURRENT_CONFIG[ox_property]["ldap_attribute"] = None
        CURRENT_CONFIG[ox_property]["alternative_attributes"] = []
    else:
        raise ValueError("Cannot unset non-nillable property")


def add_alternatives(ox_property, alternatives):
    CURRENT_CONFIG[ox_property]["alternative_attributes"] = alternatives


def dump(_args):
    print("Dump current configuration.")
    print(json.dumps(CURRENT_CONFIG, indent=4))


def modify(args):
    print("Modifying mapping")

    for attr in args.set:
        set_property(attr[0], attr[1])

    for attr in args.unset:
        unset_property(attr)

    for attr in args.set_alternatives:
        add_alternatives(attr[0], attr[1:])

    for attr in args.unset_alternatives:
        add_alternatives(attr, [])

    save()


def overwrite_default(_args):
    print("Restoring default configuration.")
    with open(
        OUTPUT_FILE,
        "w",
    ) as fd:
        json.dump(DEFAULT_MAPPING, fd, indent=4)
...
def main():
    ...
    mod = actions.add_parser("modify")
    mod.set_defaults(func=modify)
    ...
    restore_default = actions.add_parser("restore_default")
    restore_default.set_defaults(func=overwrite_default)
