...
def write_it(lo, pos, fname):
    if os.path.exists(fname):
        print(
            "{} already exists, not touching it... Remove it and rerun this script if needed".format(
                fname,
            ),
        )
        return
    with open(fname, 'w') as fd:
        ...
        for res in lo.search(
            filter=user_filter,
            attr=['oxAccess', 'oxAccessUSM'],
        ):
            dn, attrs = res
            ...
            if (
                ...
                and access_profile
                and not (
                    'USM' in access_profile and 'activeSync' in access_profile
                )
            ):
                print(
                    "Found {} with oxAccessUSM enabled and an Access profile without mobility {}".format(
                        dn,
                        ox_access,
                    ),
                )
    ...
    with open(fname) as fd:
        ...
        for line in csv_reader:
            ...
            if get_access_profile(ox_access + "_mobility", True):
                ...
                if not get_access_profile(
                    ox_access + "_mobility_migrated",
                    True,
                ):
                    ...
                    try:
                        ...
                        print(
                            "Creating Access profile {} with mobility enabled from {}".format(
                                ox_access + "_mobility_migrated",
                                ox_access,
                            ),
                        )
...
parser.add_argument(
    '--file',
    help='Filename where the state is written to or read from',
)
parser.add_argument('--binddn', help='Bind DN for LDAP connection')
parser.add_argument(
    '--bindpwdfile',
    help='Filename where the LDAP password is stored in',
)
