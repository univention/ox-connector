...
def _normalize_resource(resource):
    ...
    attrs = {
        ...
        if resource_attributes.get(key)
    }
    ...
    for ctx in Context.list():
        ...
        if obj:
            if not ret:
                ...
                ret.id = (
                    ret.id + [obj.id]
                    if isinstance(ret.id, list)
                    else [ret.id, obj.id]
                )
                ret.context_id = (
                    ret.context_id + [obj.context_id]
                    if isinstance(ret.context_id, list)
                    else [ret.context_id, obj.context_id]
                )
    return ret
...
def _get_ox_functional_account(_1, _2, name):
    ...
    for ctx in Context.list():
        objs = [
            x for x in FunctionalAccount.list(ctx.context_id) if x.name == name
        ]
        for obj in objs:
...
def get_ox_object(module, id, context=None):
    ...
    return _get_from_ox[module](
        UDM_MODULE_TO_OX_SOAP_SERVICE[module],
        context,
        id,
    )
...
def get_diff(module, file_obj, ox_obj, udm_obj):
    ...
    ignored_attributes = (
        set(['objectFlag', 'umcProperty'])
        | _specific_ignored_attributes[module]
    )
    ...
    for key in keys - ignored_attributes:
        ...
        if module == "oxmail/functional_account" and key in (
            'context_id',
            'id',
        ):
            continue
        elif (
            module in ("oxmail/functional_account", "groups/group")
            and key == 'users'
        ):
            v_file = sorted(
                res["object"]["username"]
                for dn in v_file
                if (res := get_old_listener_object(dn))
                and res["object"]["isOxUser"] == "OK"
                and not res["object"]["username"].startswith("oxadmin")
            )
            v_udm = sorted(
                res.properties["username"]
                for dn in v_udm
                if (res := get_udm_object("users/user", dn))
                and res.properties["isOxUser"]
                and not res.properties["username"].startswith("oxadmin")
            )
            ox_usernames = []
            for id, ctx in v_ox:
                soap_service = User.service(ctx)
                obj = soap_service.get_data(
                    soap_service.Type(id=id, name=None),
                )
                if obj:
            ...
            if v_file != v_ox or v_file != v_udm:
                print(
                    f"DIFF({key}): old listener value: {v_file}, ox value: {v_ox}, udm value: {v_udm}",
                )
                ok = False
        elif key == "oxAccess":
            if v_file != v_udm:
                print(
                    f"DIFF({key}): old listener value: {v_file}, udm value: {v_udm}",
                )
                ok = False
        elif key == "aliases":
            v_file = [file_obj.get("mailPrimaryAddress")] + file_obj.get(
                "mailAlternativeAddress",
                [],
            )
            v_udm = [udm_obj.get("mailPrimaryAddress")] + udm_obj.get(
                "mailAlternativeAddress",
                [],
            )
            if v_file != v_ox or v_file != v_udm:
            ...
            if v_file:
                print(
                    f"DIFF({key}): old listener value: {v_file}, ox value: {v_ox}. Replace the 'oxDbId' field in the old listener file with the current ox value.",
                )
                ok = False
...
def resync_old_obj(dn, module):
    ...
    print(
        f"Object {dn} has been scheduled to be resynced with file {new_name} and should be processed in the next connector loop.",
    )
...
def search_and_compare_by_dn(args):
    ...
    module = args.udm_module or (
        file_object.get("udm_object_type") if file_object else None
    )
    if not module:
        print("Error: can't find udm module. Specify it with --udm_module")
        sys.exit(1)

    context = args.ox_context or (
        file_object.get("object", {}).get("oxContext") if file_object else None
    )
    ...
    if not udm_object:
        print(
            "Error: Object not found in udm. The object with the specified dn does not exits. It possibly has been deleted or moved.",
        )
        sys.exit(1)
    ...
    if module == "users/user" and not udm_object.properties['isOxUser']:
        print(
            "The object is not an OX user (udm property 'isOxUser'). Enable it to synchronize.",
        )
        return
    if module == "groups/group" and not udm_object.properties['isOxGroup']:
        print(
            "The object is not an OX group (udm property 'isOxGroup'). Enable it to synchronize.",
        )
        return
    ...
    if not ox_object:
        if module == 'users/user':
            print(
                f"User {name} not found in ox database in context {context}. Looking in other contexts...",
            )
            ids = search_ox_user_duplicates(name)
            if len(ids) > 0:
                print(
                    f"User found in contexts {ids}. Check if the object is still in the listener queue. If it is not there you have to manually move the object from the ox database using the ox cli tools (Tip: use the cli tool 'usercopy' to preserve user info) and remove appearances in old contexts after that.",
                )
                if args.resync:
                ...
                print(
                    "The object wasn't synced yet, it was manually removed from the database or there was a problem during the provisioning.",
                )
        else:
            print("Object not found in ox database.")
        check_diff = False

    if not file_object:
        print(
            "Object not found in /old directory. The object wasn't synchronized yet or the directory was removed.",
        )
        check_diff = False
    ...
    result = (
        get_diff(
            module,
            file_object.get("object", {}),
            normalize_ox_attributes(ox_object, module),
            udm_object.properties,
        )
        if check_diff
        else False
    )
    if result:
        print(
            "Success: Data of the object is consistent between OX, UDM and listener files.",
        )
    elif args.resync:
        if not file_object:
            uuid = udm_object.representation['uuid']
            print(
                "Can't resync using old listener file. It doesn't exist. You can trigger a resync running the following command from your ucs system.\n\n"
        ...
        print(
            "Warning: Data inconsistencies detected. Object data differs between OX, UDM and listener files.\nResync object data by running this command again with the --resync flag.",
        )
...
def main():
    ...
    parser.add_argument(
        '--dn',
        required=True,
        help='Check the object with the specified dn.',
    )
    parser.add_argument(
        '--udm_module',
        help="Object's udm module. Required if the property is missing in the old/ directory object.",
    )
    parser.add_argument(
        '--ox_context',
        help="Object's ox context. Required if the property is missing in the old/ directory object.",
    )
    parser.add_argument(
        '--resync',
        action='store_true',
        help="Resync object data by creating a new file in the listener. Resynchronizing groups will only work if its users are correctly provisioned.",
    )
    parser.add_argument(
        '--udm_admin_account',
        help="UDM user used for connection.",
        required=True,
    )
    parser.add_argument(
        '--udm_password_file',
        help="UDM password file.",
        required=True,
    )
    ...
    check = json.loads(
        subprocess.run(
            ['/usr/local/share/ox-connector/resources/get_current_error.py'],
            capture_output=True,
            text=True,
        ).stdout,
    )
    ...
    if (
        len(
            glob.glob(
                "/var/lib/univention-appcenter/apps/ox-connector/data/listener/*.json",
            ),
        )
        > 0
    ):
        print(
            "Warning: The OX Connector queue is not empty. There may be data inconsistencies if the queried object is still in the queue.",
        )

    with open(args.udm_password_file) as f:
        global udm
        udm = udm_rest.UDM(
            f"{args.udm_host}/univention/udm/",
            args.udm_admin_account,
            f.read().strip(),
        )
