...
from univention.ox.soap.backend_base import (
    BackendMetaClass,
    Context,
    Group,
    Resource,
...
from .config import (
    ...
    remove_context_admin_password,
    get_new_context_attributes,
    DEFAULT_CONTEXT,
    OX_SOAP_SERVER,
    QUOTA,
)
...
"""
    ...
    def list(cls, context_id, pattern=None, *args, **kwargs):
        ...
        return [
            cls._soap_obj2base_obj(context_id, soap_obj)
    ...
    def create(self):  # type: () -> int
        ...
        if hasattr(self, 'display_name'):
            self.display_name = (
                self.name if self.display_name is None else self.display_name
            )
...
class SoapContext(with_metaclass(BackendMetaClass, SoapBackend, Context)):
    ...
    def create(self):  # type: () -> int
        ...
        self.login_mappings = (
            self.login_mappings or context_creation_kwargs['mapping']
        )
    ...
    def list(cls, context_id=None, pattern=None, *args, **kwargs):
        ...
        return [
            cls._soap_obj2base_obj(int(soap_obj.id), soap_obj)
            for soap_obj in soap_objs
        ]
...
class SoapSecondaryAccount(
    ...
    def create(self):
        ...
        Create object with current attributes.
        """
        obj = self.service(self.context_id).Type(
            name=self.name,
            primaryAddress=self.email,
            personal=self.personal,
            mailEndpointSource=self.mail_endpoint_source,
            login=self.login,
        )
...
class SoapDeputyPermission(
    ...
    def unblock_manual(cls, context_id, user_id):
        return cls.service(context_id).unblock_manual({"id": user_id})
