...
from univention.ox.listener_tools import (
...
groups = {}
context_objs = dict(
    (c.id, c) for c in client_context.service.listAll(auth=master_creds)
)
context_creds = dict(
    (c_id, credentials_type(*get_credentials_for_context(c_id)))
    for c_id in context_objs.keys()
)

print(
    'Found contexts: {}'.format(
        ', '.join(str(c_id) for c_id in context_objs.keys()),
    ),
)
...
print(
    'Total time for group retrieval: {:.2f} seconds.'.format(time.time() - t0),
)
...
print(
    'Retrieved {} user IDs in {:.2f} seconds.'.format(
        sum(len(g) for g in simple_users.values()),
        time.time() - t0,
    ),
)
...
for context_id, context_obj in context_objs.items():
    print(
        'Retrieving {} complete user objects in context {}...'.format(
            len(simple_users[context_id]),
            context_id,
        ),
    )
...
print(
    'Retrieved {} complete user objects in {:.2f} seconds.'.format(
        sum(len(g) for g in users.values()),
        time.time() - t1,
    ),
)
print(
    'Total time for user retrieval: {:.2f} seconds.'.format(time.time() - t0),
)
