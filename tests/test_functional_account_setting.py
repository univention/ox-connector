...
from univention.ox.provisioning.functional_account import (
    configure_functional_account_login,
)
...
def test_functional_account_default_container(
    ...
    try:
        ...
        udm.modify(
            ...
            dn,
            {
                'ox_functional_accounts': [
                    f'cn=users,{ldap_base}',
                    f'cn=groups,{ldap_base}',
                ],
            },
        )
        containers = get_default_containers(
            umc_uri,
            udm_admin_username,
            udm_admin_password,
        )
        expected = [
            f'cn=groups,{ldap_base}',
            f'cn=functional_accounts,cn=open-xchange,{ldap_base}',
            f'cn=users,{ldap_base}',
        ]
